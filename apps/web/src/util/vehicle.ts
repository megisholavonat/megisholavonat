...
export function dataAppearsFalsified(
  ...
  const realtimeStops = vehiclePosition.trip.stoptimes.filter(
    (stoptime) =>
      stoptime.realtimeArrival != null || stoptime.realtimeDeparture != null
  );
...
export function isFarFromRoute(
  ...
  const simplifiedCoords =
    geoJsonCoords.length > MAX_POINTS
      ? geoJsonCoords.filter(
          (_, index) =>
            index % Math.ceil(geoJsonCoords.length / MAX_POINTS) === 0
        )
      : geoJsonCoords;
  ...
  try {
    ...
    const nearestPoint = turf.nearestPointOnLine(routeLine, currentPosition);
